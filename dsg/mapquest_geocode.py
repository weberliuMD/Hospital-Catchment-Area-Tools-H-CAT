...
def mapquest_geocode(Address, APIKEY):
    '''
    mapquest_geocode(Address) is a function developed to query the Mapquest (Verison) geocoding API.
...
def mapquest_open_geocode(Address, APIKEY):
    '''
    mapquest_open_geocode(Address) is a function developed to query the Mapquest (OSM) geocoding API.
    This function takes in the argument of:
    Address: The address of the patient, spaces allowed - fuzzy matching using mapquest (OSM)'s Algorithm
    will clarify any issues. Provide as much detail as possible in the address.
    APIKEY: The APIKEY as per google cloud console (a sign-up will be required).
    ...
    Date Created: 25/09/2019
    '''
    mapquestREST = "https://open.mapquestapi.com/geocoding/v1/address?key="
    webAddress = mapquestREST + APIKEY + "&inFormat=kvp&outFormat=json&location=" + Address + "&thumbMaps=true"
    print("Request sent to "+ webAddress)
    r = requests.get(webAddress).json()
    # print(respData)
